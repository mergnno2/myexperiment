...
class SimpleSwitchWithScanDetection(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        self.set_collection_entry(ev=ev, table_id=0)
        self.logger.info("Table-0-collection set.")

        # Table-1 (forwarding flow table) Table-miss flow entry
        match = parser.OFPMatch()
    ...
    def _packet_in_handler(self, ev):
        ...
        if ev.msg.table_id == 0:
            # it is a suspicious flow
            self.handle_scan_detection(ev=ev)
        elif ev.msg.table_id == 1:
            # it is a normal flow
