...
def update_network_info_for_victim(flow):
    # MARK EVERY FLOWS AS NORMAL WHEN IT HAS THE FLAG 'A' AND PUT IT IN THE NETWORK INFORMATION.
    ...
    if flow[2] != "TCP  " or re.search("A", flow[10]) is None:
        return
    dst = Target(IP=flow[5], port=flow[6])
...
labels = []
for h in hosts:
    # pic, = plt.plot(host.window_ewma)
    label = h.IP
    print(h.window_ewma)
    # pics.append(plt.plot(host.window_ewma)[0])
    labels.append(str(label))
pic_0, = plt.plot(hosts[0].window_ewma, color='red')
pic_1, = plt.plot(hosts[1].window_ewma, color='yellow')
pic_2, = plt.plot(hosts[2].window_ewma, color='blue')
pic_3, = plt.plot(hosts[3].window_ewma, color='black')
pics.append(pic_0)
pics.append(pic_1)
pics.append(pic_2)
pics.append(pic_3)
